// The cards a trial is made of.
//
// Pulled out of TrialDetailPage when that page reached nineteen cards and
// thirteen phone screens. They are shared now, because the page split into
// three: an overview that answers "what now", a setup page for the work that
// happens once, and a results page for reading it back.
//
// They live here rather than in the page that used to own them so no page is
// importing from another page — and so the next card has an obvious home that
// is not "the bottom of the trial page".

import { useState } from "react";
import { Link, useNavigate } from "react-router-dom";
import { format } from "date-fns";
import { addArm, addTemplate, removeArm, removeTrial, saveArm } from "../../services/store";
import { buildEntryUrl } from "../../services/entryLinks";
import { describeEvent, describeEventScope } from "../../services/events";
import { TRIAL_STATES } from "../../services/lifecycle";
import { metricDisplay } from "../../services/metricValue";
import { type Completeness, type TreatmentStat } from "../../services/replication";
...
import { TimingEditor } from "../ObservationTiming";
...
export function RemoveTrial({ trial }: { trial: Trial }) {
 ...
 */
export function EditEntryLink({ event }: { event: MeasurementEvent }) {
...
export function TrialDesignCard({
  trial,
  templates,
  layoutLocked,
}: {
 ...
 */
export function TrialStage({ trial }: { trial: Trial }) {
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState<string | null>(null);
 ...
 */
export function AddForm({ trial }: { trial: Trial }) {
  const navigate = useNavigate();
